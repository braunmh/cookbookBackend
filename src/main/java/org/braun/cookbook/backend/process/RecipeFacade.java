...
public class RecipeFacade {
    ...
    public void delete(String id) {
        try (SolrClient client = getSolrClient()) {
            ...
            LOG.error("searchByAttributes failed", e);
        }
    }
    /**
     *
     * @param content blank separted list of words
     * @param keywords List of keyword ids
     * @param rating rating from 1 to 5
     * @param directory to search
     * @param evaluated true or false
     * @param dateFrom
     * @param dateTo
     * @return List of Recipes found or an empty List
     * @throws org.braun.cookbook.backend.process.ConditionParseException
     */
    public List<RecipeShort> searchByAttributes(String content, List<Long> keywords, Integer rating, String directory,
            Boolean evaluated, String dateFrom, String dateTo) throws ConditionParseException {
        try (SolrClient solrClient = getSolrClient()) {
            ...
            SolrQueryBuilder builder = new SolrQueryBuilder()
                    ...
                    .addQueryText("content", content)
                    .addQueryString("pathParent", directory)
                    .addQuery("evaluated", (evaluated) ? Boolean.TRUE : null)
            ...
            LOG.error("searchByAttributes failed", e);
            throw new ConditionParseException("searchByAttributes failed: " + e.getMessage());
        }
    }
    ...
    public List<Suggestion> getSuggestion(String fieldName, String value) {
        ...
        try (SolrClient client = getSolrClient()) {
            ...
            String prefix = new String(output, 0, length);
            ...
            List<TermsResponse.Term> terms = request.process(client, Configuration.getInstance().getSolrCollection()).getTermsResponse().getTerms(fieldName);
            return terms.stream().map(t -> new Suggestion(t.getTerm(), t.getFrequency())).toList();
        } catch (IOException | SolrServerException e) {
            LOG.error("Suggestion for content failed.", e);
        }
        return Collections.emptyList();
    }
    ...
    public Recipe insert(Recipe recipe, String pathParent, byte[] image) throws IOException {
        ...
        try {
            ...
            recipe.setId(String.valueOf(sequenceGenerator.getNext()));
            if (image != null) {
            ...
            LOG.error("Reading recipe " + recipeShort.getPath(), e);
    ...
    public RecipeShort findByUrl(String url) throws IOException {
        ...
        try (SolrClient solrClient = getSolrClient()) {
            ...
            return res.getFirst().toRecipeShort();
        } catch (IOException | SolrServerException e) {
            LOG.error("searchByAttributes failed", e);
            throw new IOException("findByUrl failed: " + e.getMessage());
        }
    }
     ...
     * @param output The result of the folding. Should be of size &gt;=
     * {@code length * 4}.
