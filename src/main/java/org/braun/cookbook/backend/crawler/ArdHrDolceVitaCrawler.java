...
public class ArdHrDolceVitaCrawler extends Crawler {
    ...
    public BackgroundJobType getTaskName() {
        return BackgroundJobType.ArdHrDolceVitaCrawler;
    }
    ...
    class ArticleFilter extends XMLFilterImpl {
        ...
        public void startElement(String uri, String localName, String qName, Attributes atts) throws SAXException {
            switch (step) {
                ...
                case preparations -> {
                    switch (localName) {
                        case "li", "p" -> writer.reset();
                        case "footer" -> step = Step.finished;
                    }
                }
            }
                ...
                case preparations -> {
                    switch (localName) {
                        case "p", "li" -> {
                            ...
                            recipe.getDescription().add(new Paragraph().value(heading));
                            writer.reset();
                        }
                        case "article" -> {
                            step = Step.finished;
                ...
                if ("meta".equals(localName)) {
                    switch (atts.getValue("property")) {
                        ...
                        case "twitter:image:alt" -> {
                            recipe.setTitle(atts.getValue("content"));
                        }
                        default -> {}
                    }
